...
include'../PHPCode/AdminViewBookingHandler.php';
?>
<html>
    <head>
<title>List The Booking</title>
...
<!-- Placed js at the end of the document so the pages load faster -->
   ...
   <!-- left side start-->
	...
	<!-- //header-ends -->
			<div id="page-wrapper">
				<div class="graphs">
					<h3 class="blank1">List Of Bookings</h3>
					 <div class="xs tabls">
						 ...
						 <thead>
							<tr>
							  <th>Booking ID</th>
                                                            ...
                                                            <th>Item Name</th>
                                                            <th>Item Weight</th>
                                                            <th>Number of Cargo Capacity</th>
							</tr>
						  </thead>
						  <tbody>
							<?php
                                                        if (count($results->data)>=1){
                                                            for ($i=0;$i<count($results->data);$i++){
                                                                echo '<tr>';                  
                                                                echo '<td>'.$results->data[$i]['BookingID']. '</td>';
                                                                echo '<td>'.$results->data[$i]['CompanyName']. '</td>';
                                                                echo '<td>'.$results->data[$i]['CompanyContactNumber']. '</td>';
                                                                echo '<td>'.$results->data[$i]['CompanyEmail']. '</td>';
                                                                echo '<td>'.$results->data[$i]['CompanyAddress']. '</td>';
                                                                echo '<td>'.$results->data[$i]['ItemName']. '</td>';
                                                                echo '<td>'.$results->data[$i]['ItemWeight']. '</td>';
                                                                echo '<td>'.$results->data[$i]['NumberofCargoCapacity']. '</td>'; 
                                                                echo'</tr>';  
                                                        }
                                                        }
                                                        else{
                                                            echo'<script>
                                                                alert("There are no available bookings for this schedule");
                                                                window.location.href=\'AdminViewBookings.php\';
                                                                </script>';
                                                        }
                                                        ?>
						  </tbody>
