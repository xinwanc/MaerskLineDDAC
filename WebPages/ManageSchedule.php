...
include'../PHPCode/ViewSchedule.php';
?>
<html>
    <head>
							...
							<?php
                                                        if (count($results->data)>=1){
                                                            for ($i=0;$i<count($results->data);$i++){
                                                                ...
                                                                echo '<td>
                                                                <a onclick="return confirm(\'Do you want to remove this schedule?\')" href=
                                                                "ManageSchedule.php?deleteid='.$results->data[$i]['ID'].'" class = "btn btn-danger">Delete</a>
                                                                </td>'; 
                                                                echo'</tr>';  
                                                        }
                                                        }
                                                          ...
                                                          if(isset($_GET['deleteid'])){
                                                            $FindDetails = mysqli_escape_string($loginconnect, $_GET['deleteid']);
                                                            $FindLa = mysqli_query($loginconnect, "Select* from vesselschedule where ID='$FindDetails'");                                                            
                                                            //check the arrival date is after current date or not
                                                            ...
                                                            //if the arrival date is later than the current date, user is not allowed to remove the schedule
                                                            if($DisplayRow['EstimatedDateArrival']>=$CurrentDate){
                                                                echo '<script>
                                                                alert("You are not allowed to remove the schedule as the arrival date is not over yet."); 
                                                                window.location.href=\'ManageSchedule.php\'; 
                                                                </script>';
                                                            ...
                                                            if( $SetInactive){
                                                                 echo'<script>
                                                                 alert("You have successfully remove the vessel schedule");
                                                                 window.location.href=\'ManageSchedule.php\';
                                                                </script>';
                                                            }
                                                            }
